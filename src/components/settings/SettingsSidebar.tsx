...
export const SettingsSidebar = ({ activeTab, onTabChange }: SettingsSidebarProps) => {
    return (
        <div className="flex w-full flex-col gap-3 rounded-xl border border-border-light bg-form-bg p-4 sm:w-[200px] sm:p-6">
            {TABS.map((tab) => (
                <button
                    key={tab.key}
                    ...
                    className={cn(
                        ...
                        activeTab === tab.key
                            ...
                            : 'border border-transparent text-white-secondary hover:bg-white/5 hover:text-white'
                    )}
                >
                    {tab.label}
                </button>
